	...
	render() {
		...
		let children = this.data.map((url, currentPosition) => {
			...
			let onPanend = (event) => {
				...
				let lastAnimation = new Animation(last.style, "transform", (v) => `translateX(${v}px)`, lastP1, lastP2, 500, 0,
					ease);
				let currentAnimation = new Animation(current.style, "transform", (v) => `translateX(${v}px)`, currentP1,
					currentP2, 500, 0, ease);
				let nextAnimation = new Animation(next.style, "transform", (v) => `translateX(${v}px)`, nextP1, nextP2, 500, 0,
					ease);
		...
		let divElement = < div class = "carousel" > {
			children
		} < /div>;
